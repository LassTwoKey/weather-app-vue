...
import {getWindDirection, getWindString, hPaToMmHg, splitIntoSubArrays} from "./misc.ts";
...
export const setWeather = async (lat?: number, lon?: number) => {
    ...
    if (!isNaN(+geoStore.getLat) && !isNaN(+geoStore.getLon)) {
        const weatherData:WeatherInfo | null = await getWeatherByCoordinate(lat ?? +geoStore.getLat, lon ?? +geoStore.getLon)

        if (weatherData) {
            weatherInfoStore.setWeatherInfo({
                geoName: `${weatherData.name}, ${getCountryByCode(weatherData.sys.country)}`,
                temp: `${Math.round(weatherData.main.temp)}°`,
                feelsLike: `${Math.round(weatherData.main.feels_like)}°`,
                iconUrl: `https://openweathermap.org/img/wn/${weatherData.weather[0].icon}@2x.png`,
                description: `${firstLetterUpperCase(weatherData.weather[0].description)}`,
...
export const setForecast = async (lat?:number, lon?:number) => {
    ...
    if (!isNaN(+geoStore.getLat) && !isNaN(+geoStore.getLon)) {
        const forecastData:ForecastInfo | null = await getForecastByCoordinate(lat ?? +geoStore.getLat, lon ?? +geoStore.getLon)
        ...
        if (forecast) {
            ...
            const firstPoint = forecast.findIndex(forecastElem => forecastElem.time === '0:00')
            ...
            const fiveDaysForecasts = [forecast.slice(0, 8), ...splitIntoSubArrays(forecast.slice(firstPoint), 8).slice(0, 4)]
