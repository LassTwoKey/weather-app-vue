...
const daysInfo = computed(() => forecastList.value.map((dayInfo, idx) => {
  const date = firstLetterUpperCase(getFormattedDateForFiveDays(new Date().setDate(new Date().getDate() + idx)))
  ...
  let tempMax = `${Math.max(...dayInfo.map(item => +removeDegreeFromStr(item.tempMax)))}°`
  let tempMin = `${Math.min(...dayInfo.map(item => +removeDegreeFromStr(item.tempMin)))}°`
  ...
  const mainCards = fiveDaysStore.getMainCards.map(mainCard => ({
    ...
    descr: mainCard.id === 'windSpeed' ? dayInfo[pickIndex].windDirection : null
  }))
