...
export async function getWeatherByCity(city: string) {
  ...
  const response = fetch(
    `https://api.openweathermap.org/data/2.5/weather?lang=ru&q=${city}&units=${
...
export async function getWeatherByCoordinate(lat: number, lon: number) {
  const unitStore = useUnitStore()
  const response = fetch(
    `https://api.openweathermap.org/data/2.5/weather?lang=ru&lat=${lat}&lon=${lon}&units=${
      getUnit() ?? unitStore.getUnit
    }&appid=${apiKey}`
  )
  return (await response).json()
...
export async function getForecastByCoordinate(lat: number, lon: number) {
  const unitStore = useUnitStore()
  const response = fetch(
    `https://api.openweathermap.org/data/2.5/forecast?lang=ru&lat=${lat}&lon=${lon}&lang=ru&units=${
      getUnit() ?? unitStore.getUnit
    }&appid=${apiKey}`
  )
  return (await response).json()
